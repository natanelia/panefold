...
assert.ok(browser.stats.expected >= 56, "The full preview browser suite must execute");
assert.equal(unit.success, true);
assert.equal(unit.numFailedTests, 0);
assert.equal(unit.numPendingTests, 0);
assert.ok(unit.numPassedTests >= 542, "The full unit suite must execute");
...
const sourceDigests = async (name) => {
  ...
  const paths = new Set([
    ...
    "packages/react/src/drop-modifiers.ts",
    "packages/react/src/tab-insertion.ts",
    "packages/react/test/drag-parity.test.ts",
    "packages/react/test/tab-insertion.test.ts",
    "e2e/drag-parity.spec.ts",
